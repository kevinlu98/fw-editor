...
require('./date.extend')
require('./jquery.textarea.extend')
require('../css/a11y-light.css')
...
export class Editor {
    ...
    menu_all = {
        undo: {
            ...
            call: () => {
                ...
                let md = this.undoStack.pop()
                this.redoStack.push(md)
                md = this.undoStack.pop()
                if (md) {
                    this.element.val(md)
            ...
            call: () => {
                ...
                if (!this.init) {
                    ...
                    $('#fw-editor-cn .fw-menu-headlist li').on('click', function () {
                        ...
                        _this.replaceSelection(text, parseInt($(this).data('level')) + 2)
                    })
                    this.init = true
                }
        ...
        code: {
            ...
            call: () => {
                ...
                this.dialogBox({
                    ...
                    right: () => {
                        let text = '\n```' + $("#fw-language").val() + '\n\n```\n'
                        this.replaceSelection(text, $("select#fw-language").val().length + 5)
                        this.closeDialog()
                    }
                })
            }
        ...
        views: {
            ...
            call: () => {
                console.log(this.element.width(), this.container.width() / 2);
                if (this.element.width() > this.container.width() / 2) {
                    this.element.css('flex', '0 0 50%')
                } else {
                    this.element.css('flex', '0 0 100%')
                }
    ...
    constructor(id = "", options = {}) {
        ...
        this.settings['view'] = options['view'] ? options['view'] : false;
        if (options['menuExt']) {
        ...
        this.settings['history'] = options['history'] ? options['history'] : 10

        this.undoStack = new Stack(this.settings['history'] * 2)

        this.redoStack = new Stack(this.settings['history'])
        this.settings['menu'] = options['menu'] ? options['menu'] : Object.keys(this.menu_all)
        this.element.parent().append('<div id="fw-editor-cn"><div class="fw-tool-bar"></div>' +
            '<div class="fw-editor-content"><div class="line-number"><span>1</span></div></div>' +
            '<div id="fw-dialog-mask">' +
            '<div class="fw-dialog-window">' +
            '<div class="title">' +
            '<span></span>' +
            '<svg t="1639569410622" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="11148" width="200" height="200"><path d="M512 1023.999248a510.488984 510.488984 0 1 1 199.297091-40.28374 508.810495 508.810495 0 0 1-199.297091 40.28374z m0-964.903001C262.240811 59.096247 59.089909 262.274005 59.089909 512.006338s203.177758 452.910091 452.910091 452.910091 452.910091-203.177758 452.910091-452.923519S761.745761 59.082819 512 59.082819z" fill="#474747" p-id="11149"></path><path d="M548.980474 516.826959l180.820281-180.753142a29.541409 29.541409 0 0 0-41.774238-41.774239L507.206235 475.11986 326.399381 294.299578a29.553495 29.553495 0 0 0-41.814522 41.774239l180.847137 180.753142-180.847137 180.873993a29.541409 29.541409 0 0 0 41.774238 41.774239l180.847138-180.806854 180.820282 180.820282a29.541409 29.541409 0 0 0 41.774238-41.774239z" fill="#474747" p-id="11150"></path></svg>' +
            '</div>' +
            '<div class="fw-dialog-body"></div>' +
            '<div class="fw-dialog-footer"></div>' +
            '</div>' +
            '</div>')
        this.container = $("#fw-editor-cn")
        this.lineContent = $("#fw-editor-cn .line-number")
        this.toolBar = $("#fw-editor-cn .fw-tool-bar")
        this.container.children('.fw-editor-content').append(this.element)
        this.container.children('.fw-editor-content').append('<div id="write"><div class=""></div>')
        this.viewer = $("#write")
        marked.setOptions({
        ...
        //         let width = start - mouseX - 3;
        //         if (width < 0) {
        //             _this.element.css('flex', `0 0 100%`)
        //         } else {
        //             _this.element.css('flex', `0 0 ${_this.container.width() - width}px`)
        //         }
        //     }
        // })
        this.element.bind('input propertychange', () => {
            this.lineNumber()
        }).scroll(() => {
            this.lineContent.scrollTop(this.element.scrollTop())
            $("#write").scrollTop(this.viewer.height() * (this.element.scrollTop() / this.element.height()))
        })
        this.element.on('keydown', (e) => {
            if (e.keyCode === 9) {
                e.preventDefault();
                this.replaceSelection('    ', 4)
            }
        })
        this.lastmd = this.element.val()
        setInterval(() => {
            this.lineNumber()
            let md = this.element.val()
            if (md !== this.lastmd) {
                this.lastmd = md
                this.undoStack.push(md)
                let ht = marked.parse(md)
                if (options['parse']) {
                    ht = options.parse(ht)
                }
                if (ht !== this.viewer.html()) {
                    this.viewer.html(ht)
                }
            }
        }, 1000)
    ...
    closeDialog() {
        $("#fw-dialog-mask").fadeOut()
        $("#fw-dialog-mask .fw-dialog-window .fw-dialog-footer").html('')
        $("#fw-dialog-mask .fw-dialog-window .title span").html('')
        $("#fw-dialog-mask .fw-dialog-window .fw-dialog-body").html('')
    }

    dialogBox(option = {}) {
        let title = option['title'] ? option['title'] : '弹出层';
        let width = option['width'] ? option['width'] : 200;
        let height = option['height'] ? option['height'] : 150;
        let content = option['content'] ? option['content'] : '';

        if (option['cancel']) {
            $("#fw-dialog-mask .fw-dialog-window .fw-dialog-footer").append('<a class="fw-dialog-btn fw-btn-cancel" href="javascript:void(0)">取消</a>')
            $("#fw-dialog-mask .fw-btn-cancel").on('click', () => {
                this.closeDialog()
            })
        }
        if (option['right']) {
            $("#fw-dialog-mask .fw-dialog-window .fw-dialog-footer").append('<a class="fw-dialog-btn fw-btn-right" href="javascript:void(0)">确认</a>')
            $("#fw-dialog-mask .fw-btn-right").on('click', option['right'])
        }

        $("#fw-dialog-mask .fw-dialog-window .title span").html(title)
        $("#fw-dialog-mask .fw-dialog-window .fw-dialog-body").html(content)
        $("#fw-dialog-mask .fw-dialog-window").css('width', width + 'px').css('height', height + 'px')
        $("#fw-dialog-mask").fadeIn()
    ...
    selection() {
        ...
        let value = dom.value
        return value.substring(consor.start, consor.end);
    }

    replaceSelection(text, pos) {
        let dom = this.element[0];
        let consor = this.consor();
        let value = dom.value
        let newText = value.substring(0, consor.start) + text + value.substring(consor.end, value.length);
        this.element.val(newText)
        dom.setSelectionRange(consor.end, consor.end)
        if (pos) {
            if (consor.start === consor.end) {
                this.element.setCurPos(consor.start + pos)
            } else {
                this.element.setCurPos(consor.start + text.length)
            }
        } else {
            this.element.setCurPos(consor.start + text.length)
        }
    ...
    lineNumber() {
        let num = this.element.val().split('\n').length
        if (num > this.lineNum) {
            let addline = num - this.lineNum;
            for (let i = 0; i < addline; i++) {
                let number = this.lineNum + i + 1;
                this.lineContent.append(`<span>${number}</span>`)
            }
        }
        if (num < this.lineNum) {
            this.lineContent.html('')
            for (let i = 0; i < num; i++) {
                let number = i + 1;
                this.lineContent.append(`<span>${number}</span>`)
            }
    ...
    reload() {
        ...
        this.viewer.css('height', this.settings['height'] + 'px')
        this.buildToolBar()
    }
}